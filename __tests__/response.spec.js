...
const healthyResponse = Object.assign(
  {},
  baseResponse,
  {
    healthy: true
...
const unhealthyResponse = Object.assign(
  {},
  baseResponse,
  {
    healthy: false,
...
describe('Response', () => {
  describe('Validation', () => {
    ...
    const dependentOnRequiredForTypes = [types.INFRASTRUCTURE, types.INTERNAL_DEPENDENCY, types.EXTERNAL_DEPENDENCY]
    describe('dependentOn is required for type', () => {
      dependentOnRequiredForTypes.forEach((type) => {
        test(type, () => {
          expect(() => response(Object.assign({}, unhealthyResponse, { type }))).toThrow()
        })
      })
    })
    ...
    describe('dependentOn should not be present for type', () => {
      R.keys(R.omit(dependentOnRequiredForTypes, types)).forEach((type) => {
        test(type, () => {
          expect(() => response(Object.assign({}, healthyResponse, { dependentOn: 'foobar' }))).toThrow()
        })
      })
    })
  })
  ...
  describe('serialization', () => {
    ...
    test('should format dependentOn as an object', () => {
      const actual = serializeResponse(Object.assign({}, unhealthyResponse, { type: types.INFRASTRUCTURE, dependentOn: 'foo' }))
      const expected = {}
    ...
    test('should remove empty fields', () => {
      const fields = R.keys(serializeResponse(Object.assign({}, unhealthyResponse, { info: undefined, link: '' })))
